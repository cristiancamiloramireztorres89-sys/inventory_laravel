{{-- ========================================================================= --}}
{{-- MODAL REGISTRAR VENTA MULTI-PRODUCTO (VENDEDOR) --}}
{{-- ========================================================================= --}}
<div id="modalCrearVentaVendedor" class="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm hidden flex items-center justify-center p-3 sm:p-4">
    <div class="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-xl w-full flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-150 overflow-hidden">
        ...
        <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 flex-shrink-0 bg-white">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <h3 class="font-bold text-slate-900 text-sm sm:text-base leading-tight">Registrar Venta</h3>
                    <p class="text-[11px] text-slate-400">Punto de Venta Mostrador</p>
                </div>
            </div>
            <button type="button" onclick="cerrarModalCrearVentaVendedor()" class="w-8 h-8 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-700 flex items-center justify-center text-base cursor-pointer transition-colors">
                <i class="bi bi-x-lg"></i>
            </button>
        </div>

        {{-- FORMULARIO CON BODY SCROLLABLE Y FOOTER FIJO --}}
        <form action="{{ route('vendedor.ventas.store') }}" method="POST" id="formCrearVentaVendedor" class="flex flex-col flex-1 overflow-hidden" autocomplete="off" onsubmit="return validarEnvioVentaVendedor()">
            @csrf
            ...
            <div class="px-5 py-3.5 space-y-3 overflow-y-auto flex-1">
                ...
                <div class="space-y-1 relative">
                    <div class="flex items-center justify-between">
                        ...
                        <button type="button" onclick="activarModoNuevoClienteVendedor()" class="text-[11px] font-bold text-indigo-600 hover:text-indigo-800 transition-colors cursor-pointer">
                            + Registrar Nuevo Cliente
                        </button>
                    </div>

                    <input type="hidden" name="id_cliente" id="vendedor_id_cliente" value="{{ $clientes->first()->id_cliente ?? '' }}">

                    <div id="vendedor_cliente_selector_container" class="relative">
                        <div class="relative flex items-center">
                            ...
                            <input
                                type="text"
                                id="vendedor_buscador_cliente_input"
                                placeholder="Buscar cliente por nombre, teléfono o correo..."
                                value="{{ $clientes->first() ? $clientes->first()->nombre . ($clientes->first()->telefono ? ' (' . $clientes->first()->telefono . ')' : '') : '' }}"
                                autocomplete="off"
                                onfocus="mostrarDropdownClienteVendedor()"
                                oninput="filtrarListaClienteVendedor(this.value)"
                            ...
                            >
                            <button type="button" onclick="toggleDropdownClienteVendedor()" class="text-slate-400 hover:text-slate-600 text-xs cursor-pointer" style="position: absolute; right: 12px; top: 50%; transform: translateY(-50%);">
                                <i class="bi bi-chevron-down" id="vendedor_cliente_chevron"></i>
                            </button>
                        </div>

                        {{-- Dropdown clientes --}}
                        <div id="vendedor_dropdown_lista_clientes" class="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-50 max-h-40 overflow-y-auto hidden divide-y divide-slate-100">
                            <div id="vendedor_items_clientes">
                                @foreach($clientes as $cli)
                                    <div
                                        class="vendedor-cliente-item px-3.5 py-2.5 hover:bg-slate-50 cursor-pointer flex items-center justify-between transition-colors text-xs"
                                        ...
                                        onclick="seleccionarClienteVendedor({{ $cli->id_cliente }}, '{{ addslashes($cli->nombre) }}', '{{ addslashes($cli->telefono ?? "") }}')"
                                    >
                                        <div>
                                        ...
                                        <span class="text-emerald-600 text-sm {{ $loop->first ? '' : 'hidden' }} check-vendedor-icon" id="check_vendedor_{{ $cli->id_cliente }}">
                                            <i class="bi bi-check2"></i>
                                        </span>
                                    </div>
                                @endforeach
                            </div>
                            <div id="vendedor_opcion_nuevo_cliente"
                                 ...
                                 onclick="activarModoNuevoClienteVendedor()">
                    ...
                    <div id="vendedor_box_nuevo_cliente" class="hidden space-y-2 p-3 bg-indigo-50/70 border border-indigo-100 rounded-xl mt-1.5 animate-in fade-in duration-150">
                        <div class="flex items-center justify-between">
                            ...
                            <button type="button" onclick="cancelarModoNuevoClienteVendedor()" class="text-[10px] text-slate-500 hover:text-slate-800 cursor-pointer">
                                Cancelar
                            </button>
                        </div>
                        <input type="text" name="nuevo_cliente_nombre" id="vendedor_nuevo_cliente_nombre" placeholder="Nombre completo *"
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            <input type="text" name="nuevo_cliente_telefono" id="vendedor_nuevo_cliente_telefono" placeholder="Teléfono"
                                class="w-full bg-white border border-indigo-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-600">
                            <input type="email" name="nuevo_cliente_correo" id="vendedor_nuevo_cliente_correo" placeholder="Correo"
                                class="w-full bg-white border border-indigo-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:border-indigo-600">
                        </div>
                    </div>
                </div>
                ...
                <div class="p-3.5 bg-slate-50 border border-slate-200 rounded-xl space-y-2">
                    <div class="flex items-center justify-between">
                        ...
                        <span id="vendedor_stock_badge" class="text-[10px] font-bold text-slate-500">Stock: Selecciona producto</span>
                    </div>

                    <div class="flex items-center gap-2.5">
                        {{-- Buscador interactivo --}}
                        <div class="relative flex-1" id="vendedor_producto_selector_container">
                            <input type="hidden" id="vendedor_pos_id_producto_seleccionado" value="">
                            <input type="hidden" id="vendedor_pos_precio_seleccionado" value="0">
                            <input type="hidden" id="vendedor_pos_stock_seleccionado" value="0">
                            <input type="hidden" id="vendedor_pos_nombre_seleccionado" value="">

                            <div class="relative flex items-center">
                                <i class="bi bi-search text-slate-400 text-xs pointer-events-none" style="position: absolute; left: 12px; top: 50%; transform: translateY(-50%);"></i>
                                <input
                                    type="text"
                                    id="vendedor_pos_buscador_producto"
                                    placeholder="Buscar producto por nombre..."
                                    autocomplete="off"
                                    onfocus="mostrarDropdownProductosVendedor()"
                                    oninput="filtrarListaProductosVendedor(this.value)"
                                    onkeydown="handleKeydownProductoVendedor(event)"
                                ...
                                <button
                                    type="button"
                                    id="vendedor_btn_limpiar_producto"
                                    onclick="limpiarSeleccionProductoVendedor()"
                            ...
                            <div id="vendedor_dropdown_lista_productos" class="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 max-h-48 overflow-y-auto hidden divide-y divide-slate-100">
                                <div id="vendedor_items_productos">
                                    @foreach($productos as $prod)
                                        <div
                                            class="vendedor-prod-item px-3 py-2 hover:bg-slate-50 cursor-pointer flex items-center justify-between transition-colors text-xs {{ $prod->stock_actual <= 0 ? 'opacity-50 pointer-events-none bg-slate-50/50' : '' }}"
                                            ...
                                            onclick="seleccionarProductoVendedor({{ $prod->id_producto }}, '{{ addslashes($prod->nombre) }}', {{ $prod->precio_venta }}, {{ $prod->stock_actual }})"
                                        >
                                ...
                                <div id="vendedor_prod_no_results" class="px-3.5 py-3 text-center text-xs text-slate-400 hidden">
                        ...
                        <div class="w-20 flex-shrink-0">
                            <input type="number" id="vendedor_pos_cantidad" min="1" value="1" placeholder="Cant."
                                onkeydown="if(event.key === 'Enter'){ event.preventDefault(); agregarProductoAlCarritoVendedor(); }"
                        ...
                        {{-- Botón Agregar --}}
                        <button type="button" onclick="agregarProductoAlCarritoVendedor()"
                ...
                <div class="space-y-1.5">
                    <div class="flex items-center justify-between">
                        ...
                        <span id="vendedor_resumen_items_count" class="text-[11px] font-semibold text-slate-400">0 productos</span>
                    </div>
                    ...
                    <div class="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-2xs">
                        <div class="max-h-40 overflow-y-auto relative">
                            <table class="w-full text-xs text-left" id="vendedor_tabla_carrito">
                                ...
                                <tbody id="vendedor_carrito_tbody" class="divide-y divide-slate-100">
                                    <tr>
                ...
                <div id="vendedor_hidden_items_inputs"></div>
            </div>
            ...
            <div class="px-5 py-3 border-t border-slate-100 bg-slate-50/80 flex items-center justify-between flex-shrink-0 rounded-b-2xl">
                <div>
                    ...
                    <span id="vendedor_venta_total_preview" class="text-xl font-black text-emerald-700 leading-none">$0.00</span>
                </div>
                <div class="flex items-center gap-2">
                    <button type="button" onclick="cerrarModalCrearVentaVendedor()"
                    ...
                    <button type="submit" id="vendedor_btn_confirmar_venta"
