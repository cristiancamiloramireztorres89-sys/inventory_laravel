...
@section('title', 'Ganancias y Rentabilidad | Inventory System')
@section('page_title', 'Reporte de Ganancias y Utilidad')
@section('page_subtitle', 'Supervisión de ingresos, costo de adquisición a proveedores y margen de utilidad neta')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h3 class="text-lg font-extrabold text-slate-900">Métricas de Rentabilidad Global</h3>
            <p class="text-xs text-slate-500">Balance comparativo entre costos a proveedores y precios facturados</p>
        </div>
        <span class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-slate-900 text-white text-xs font-semibold shadow-sm">
            <i class="bi bi-shield-check text-emerald-400"></i> Supervisión Global
        </span>
    </div>

    {{-- Tarjetas Resumen de Ganancias --}}
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center justify-between hover:border-slate-300 transition-colors">
            <div class="space-y-1">
                <p class="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Total Facturado (Clientes)</p>
                ...
                <p class="text-[11px] text-blue-600 font-semibold flex items-center gap-1">
                    <i class="bi bi-arrow-up-right"></i> Dinero total cobrado
                </p>
            </div>
        ...
        {{-- Costo de Compra (Inversión) --}}
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex items-center justify-between hover:border-slate-300 transition-colors">
            <div class="space-y-1">
                <p class="text-[11px] font-bold text-slate-500 uppercase tracking-wider">Costo Compra (Proveedores)</p>
                ...
                <p class="text-[11px] text-amber-600 font-semibold flex items-center gap-1">
                    <i class="bi bi-arrow-down-left"></i> Invertido en mercancía
                </p>
            </div>
        ...
        <div class="bg-white border border-emerald-200/80 rounded-2xl p-5 shadow-sm flex items-center justify-between bg-gradient-to-br from-white via-emerald-50/20 to-emerald-50/40 hover:border-emerald-300 transition-colors">
            <div class="space-y-1">
                ...
                <p class="text-[11px] font-bold {{ $gananciaTotal >= 0 ? 'text-emerald-700' : 'text-rose-700' }} flex items-center gap-1">
                    <i class="bi bi-check2-circle"></i> {{ $gananciaTotal >= 0 ? 'Dinero libre ganado' : 'Pérdida en el período' }}
                </p>
            </div>
    ...
    {{-- Top Productos con Mayor Ganancia --}}
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm space-y-3">
        <div class="flex items-center justify-between">
            <h4 class="text-xs font-extrabold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                <i class="bi bi-trophy-fill text-amber-500"></i> Top Productos con Mayor Ganancia
            </h4>
            <span class="text-[11px] text-slate-400 font-medium">Mayor utilidad acumulada</span>
        </div>
    ...
    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
            ...
            <p class="text-xs text-slate-500">
                Total de ventas evaluadas: <span class="font-bold text-slate-800">{{ $ventas->count() }}</span>
            </p>
        </div>
                ...
                <tbody class="divide-y divide-slate-100 text-xs">
                    @forelse ($ventas as $venta)
                        ...
                        <tr class="hover:bg-slate-50 transition-colors"
                            ...
                            {{-- Vendedor --}}
                            <td class="px-5 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 font-semibold text-[11px] border border-slate-200">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-12 text-center text-slate-400">
                                ...
                                No hay ventas registradas en el sistema para calcular ganancias.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<script>
    function filtrarGanancias() {
        ...
        filas.forEach(f => {
            ...
            f.style.display = (c.includes(query) || u.includes(query) || items.includes(query)) ? '' : 'none';
        });
    }
</script>
