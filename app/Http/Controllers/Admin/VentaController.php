...
{
    /**
     * Listar todas las ventas del sistema para el Administrador con vendedor responsable.
     */
    public function index(Request $request): View
    {
        ...
        return view('admin.ventas', compact('ventas', 'totalVentas', 'totalDinero', 'clientes', 'productos'));
    }

    /**
     * Registrar una nueva venta (uno o múltiples productos) y descontar existencias del inventario.
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        if ($nuevaVenta->cliente && !empty($nuevaVenta->cliente->correo) && filter_var($nuevaVenta->cliente->correo, FILTER_VALIDATE_EMAIL)) {
            try {
                ...
                // Si ocurre algún detalle en el envío de correo, no bloqueamos la venta registrada
            }
        }
    ...
    /**
     * Eliminar una venta y restaurar automáticamente el stock de los productos.
     */
    public function destroy(Venta $venta): RedirectResponse
    {
        ...
        return redirect()->route('admin.ventas')
            ->with('success', 'Venta eliminada exitosamente y stock restaurado en el inventario.');
    }
}
